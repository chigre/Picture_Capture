...
- 【常规边缘词头】提高普通拉丁词典召回：继续以“左缘 + 词性/词后结构”为主判据，但针对扫描中小号斜体词性易被 OCR 破坏的情况，新增严格左缘 + 明显粗体 + 正常行高 + 足够 OCR 置信度的保守视觉补救；定义续行仍不会被放宽。同时 POS 缩写允许 OCR 在点号处插入/漏掉空格（如 `v.tr.`→`v. tr.`/`vtr.`、`s.m.`→`s. m.`），并在 Project Profile 多页诊断中显示“强粗体左缘补救”数量。
