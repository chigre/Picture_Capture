## v2.14.0
- 主界面左侧底部功能区收敛为单行四个绿色入口：【项目中心】【初始Profile】【设置中心】【帮助中心】；原【已有项目】统一更名为【项目中心】，【新建项目】移动到项目中心搜索栏最右侧，底部不再保留【保存参数】等第二行入口。
- 【导出训练标记包】从底部功能区移动到【五、后期词典制作】最后一行；原独立【切图设置】窗口撤除并整合为【设置中心 → 切图设置】页签，沿用同一 `_CropSettings.json` 与 Section 优先规则，同时纳入设置中心自动保存。
- 原底部【图片后缀】输入框移动到【设置中心 → 项目 / 批量 → 运行与性能】，避免底部功能区承担项目参数编辑。
- 【设置中心 → 普通画线（备用） → 版面行为】调整新项目默认勾选：开启【自动精修横线Y】【使用自动版面参数】，而【跟随栏左缘倾斜/弯曲】默认关闭，自动版面子项仅默认勾选【正文起始Y】【首栏X】；【分栏数】【单栏宽】【栏间空】【单行高】【行间空】默认关闭。七个子项改为两行紧凑排布：首行 3 项、次行 4 项；不新增旧项目迁移逻辑。
- 【跟随栏左缘倾斜/弯曲】的三个调节量从固定原图 px 改为页面相对比例：搜索范围默认 **5% 单栏宽**，分块高度默认 **3% 正文有效高度**，最大局部斜率默认 **8% 分块高度**，避免不同 DPI 或逐页版面变化导致同一参数失真。
- 主界面【一、版面参数】不再展开【使用自动版面参数】的 7 个子复选框；这些字段选择统一移到【设置中心 → 普通画线（备用） → 版面行为】。主界面将【自动精修横线Y（普通/OCR共用）】简化为【自动精修横线Y（通用）】，并把【使用自动版面参数】紧随其后。
- 修复主界面【三、辅助选项及框线色块】中【隐藏线框(插图除外)】左侧多余 8px 占位，使该行与其他选项左对齐。
- 主界面【一、版面参数】移除【正文结束Y】。普通页面的有效下界改为直接使用原图高度；Project Profile 明确设置页底时使用 `height × (1-页底百分比)`；存在页面 SECTION 时再由 SECTION 边界限定。历史 `bottom_y` 不再作为 `profile_footer_mode=auto` 的隐式页底，也不再被【检测版面参数】写回。
- 旧【手动分栏】流程退出普通画线，改为【使用自动版面参数】。开启后，每页普通画线前先运行本页版面检测，并按主界面子复选框选择性更新 **分栏数 / 正文起始Y / 首栏X / 单栏宽 / 栏间空 / 单行高 / 行间空** 后再执行 VB `Draw_Auto`；这些值仅对当前页临时生效，不会污染下一页。
- `manual_y/manual_columns` 继续作为旧设置兼容字段读取，但不再影响普通画线运行；【跟随栏左缘倾斜/弯曲】仍独立作用于检测后的页面特异栏几何。
- 【普通画线】按 2016 VB.NET `Draw_Auto` 重新实现完整核心流程，并明确只使用**全分辨率原图 X/Y**：动态栏左 `LX(y)` →【微调判距】逐像素黑锚点 → 原版 `IsPoint` 二维确认 → 按【向上比例 1/x】向上找分隔 → 先找无黑点横行 → 再按 999→700 的原版 0–1000 白度阈值逐级放宽 →【白带修正量】校正 → 得到 VB 分隔 Y → 按“分隔 Y + 行高×1.2”继续扫描。普通候选/Y 定位不再经过缩小分析图或坐标缩放。
- 恢复普通画线历史参数的真实算法语义：【微调判距】不再被【正文缩进】截断；【正文缩进】只负责 `IsPoint` 的“正文缩进 × 2×正文缩进”二维区域；【黑色阈值】【候选区域白度上限】【全白阈值上限】【白度阈值下限】【白带修正量】【向上比例】【检测后跳步】均重新参与 `Draw_Auto` 对应步骤。
- 将 VB 的【向右比例 1/x】恢复为独立参数 `ordinary_right_divisor`（默认 1.0，白带分析宽度≈栏宽/该值×0.98），不再与现有 `right_ratio=100%` 的词条框/切图宽度语义混用；旧版未迁移配置在首次读取时会保留原 divisor 后再迁移现代 `right_ratio`。
- 撤销“按字高自动推导正文缩进/微调判距”的启发式改动，新项目【微调判距】恢复 VB 原始默认 5；已有项目保存的 20 等自定义值保持不变。【检测版面参数】不再擅自覆盖这两个普通画线参数。
- 【跟随栏左缘倾斜/弯曲】继续作为现代增强，为 VB 候选扫描提供每个原图 Y 对应的动态 `LX(y)`；【自动精修横线Y】改为仅在 VB 已找到分隔 Y 后做受限的小范围二次修正，不再替代 VB 自己的初始白带定位。
