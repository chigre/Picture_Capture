...
- 针对 CNIT“固定符号开头”版式新增图像级 `○/●` 标记检测：即使 OCR 漏掉或误识别圆点，也会按近圆形、空心/实心密度和稳定栏左标记带与同高 OCR 行配对，再恢复词头；同时修复“`○呵` 这类单字+固定符号”被错误当成“大字单字”而要求字号突出的逻辑。对上传的 `CNIT_0013–0032` 共 20 页图集做图像级复测后，严格圆点检测得到 886 个候选，全部落在 `.pdic` 词条 Y±40 px 内，未匹配候选为 0。
...
- 【项目 Profile → 正文与分栏】新增栏左线人工微调：在右侧预览点击栏左线选择，选中线以橙色高亮，可逐像素左移/右移并重置当前或全部；修正值按栏独立保存为参考页规范坐标，并统一作用于后续 OCR、画线、排序与切图几何。
...
- 设置中心精简版面参数标签，去掉重复的“（参考页规范坐标）”后缀；OCR画线识别策略调整为 PaddleOCR 主识别 → Tesseract 对照 → 多 OCR 自动融合，其中新项目默认仅勾选 PaddleOCR；【词头左缘容差】与【横线与文字安全距离】单位统一显示为“参考页规范px”。
