...
- 【项目 Profile → 本词典固定词头符号集】新增“本词典视觉标记样本”：可直接从真实扫描页框选入口标记/括号起始并保存为项目级归一化模板，后续按“字符符号集 → 视觉模板 → 通用形状 family”联合识别；支持模板优先、按角色合并/按具体符号区分、匹配阈值与诊断分数。连续输入如 `●○◉◯` 现在会自动拆成 4 个有效符号，并在界面显示当前生效符号集。模板保存于项目设置及 v3 Profile sidecar，不依赖原页面绝对路径，复制项目后仍可继续使用。
