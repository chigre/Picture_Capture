...
- 【项目 Profile → 词头结构】升级为完整的“词头前 + 词头本体 + 词头后”结构定义：除普通左缘词、括号词、大字单字、固定符号和编号前缀外，用户现在可分别勾选 **POS、词形/屈折、变体/性数、发音/音标、描述型结构** 是否可作为词后证据，并可独立选择“普通左缘词至少命中一种词后结构”及“词后结构 OCR 失败时允许严格左缘 + 明显粗体视觉补救”。候选 gate、分数和诊断只使用用户勾选的词后证据，不再把 POS/inflection/descriptor 隐藏揉成一个 `require_pos_or_symbol` 开关；v3 Profile sidecar 同步保存完整结构和符号集。针对常规拉丁词典，小号斜体 POS 的点号空格 OCR 变体（如 `v.tr.`→`v. tr.`/`vtr.`、`s.m.`→`s. m.`）也可识别，多页诊断会显示“强粗体左缘补救”数量。
