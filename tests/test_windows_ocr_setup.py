...
def windows_support():
    return SETUP.platform_support("Windows", "AMD64")
...
def test_gpu_recommendation_uses_highest_compatible_declared_profile():
    assert SETUP.recommended_profile(gpu_info(cuda=(13, 0)), windows_support())["extra"] == "ocr-gpu-cu129"
    assert SETUP.recommended_profile(gpu_info(cuda=(12, 9)), windows_support())["extra"] == "ocr-gpu-cu129"
    assert SETUP.recommended_profile(gpu_info(cuda=(12, 8)), windows_support())["extra"] == "ocr-gpu-cu126"
    assert SETUP.recommended_profile(gpu_info(cuda=(12, 5)), windows_support())["extra"] == "ocr-gpu-cu118"


def test_gpu_recommendation_requires_documented_compute_capability_over_7_5():
    assert SETUP.recommended_profile(gpu_info(compute_cap=(8, 0)), windows_support())["extra"] == "ocr-gpu-cu129"
    assert SETUP.recommended_profile(gpu_info(compute_cap=(7, 5)), windows_support())["extra"] == "ocr-cpu"
    assert SETUP.recommended_profile(gpu_info(compute_cap=(7, 0)), windows_support())["extra"] == "ocr-cpu"
    assert SETUP.recommended_profile(gpu_info(compute_cap=None), windows_support())["extra"] == "ocr-cpu"


def test_gpu_recommendation_falls_back_to_cpu_when_cuda_is_unknown_or_too_old():
    assert SETUP.recommended_profile(None, windows_support())["extra"] == "ocr-cpu"
    assert SETUP.recommended_profile(gpu_info(cuda=None), windows_support())["extra"] == "ocr-cpu"
    assert SETUP.recommended_profile(gpu_info(cuda=(11, 7)), windows_support())["extra"] == "ocr-cpu"
...
def test_primary_gpu_uses_index_zero_for_runtime_recommendation():
    ...
    assert SETUP.recommended_profile(info, windows_support())["extra"] == "ocr-cpu"


def test_enter_accepts_gpu_recommendation(monkeypatch):
    monkeypatch.setattr(SETUP, "platform_support", windows_support)
