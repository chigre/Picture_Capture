...
class SettingsDialog(tk.Toplevel):
    ...
    SETTING_HELP = {
        ...
        "horizontal_tolerance": "作用：恢复 VB.NET 普通画线里的【微调判距】。程序只在当前 Y 的动态栏左缘向右这一小段寻找词头黑色锚点；运行时还会自动限制它不越过【正文缩进】，避免释义正文进入候选起点通道。\n\n调整：太小会漏掉略微右移、斜体或栏线估计有轻微偏差的词头；太大会削弱栏左先验，但即使填得很大也不会超过正文缩进边界。",
        ...
        "dark_area_percent": "作用：对应 VB.NET 的 IsPoint 二维确认。找到栏左黑色锚点后，程序检查其右侧、以【正文缩进】为宽度的较大矩形；矩形平均亮度必须不高于该百分比，才说明这里确实有一块文字而不是孤立污点。默认 90%。\n\n调整：数值越高越宽松；降低会要求区域内有更多真实墨迹。通常 85%–93% 已覆盖大多数黑白扫描。",
        "row_step_multiplier": "作用：对应 VB.NET 画出一个词头后向下跳过约多少个典型行高再继续搜索。默认 1.2，可避免同一个词头的多个笔画或紧邻正文被重复识别，同时不会改变最终横线 Y。\n\n调整：重复画线时可适当增大；连续的一行一词条被漏掉时可减小。通常保持 1.0–1.3。",
    ...
    SETTING_SPIN = {
        ...
        "dark_area_percent": (1, 100, 1), "row_step_multiplier": (0.5, 3.0, 0.1),
