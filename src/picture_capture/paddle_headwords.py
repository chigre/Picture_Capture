...
def _binary_rle_components(
    ...
    """Small dependency-free 8-connected components for visual marker analysis."""
...
def _visual_marker_shape_metrics(mask: np.ndarray) -> dict[str, float]:
    """Shape evidence for an open/filled circular entry marker."""
    mask = np.asarray(mask, dtype=bool)
...
def _detect_visual_entry_markers(
    ...
    *,
    ...
    """Detect CNIT-style ○/● markers from pixels, independent of OCR text."""
    if gray.size == 0 or gray.ndim != 2:
        return []
    ...
    threshold = _otsu_threshold(roi)
    dark = roi <= threshold
    components = _binary_rle_components(dark)
    markers: list[dict[str, Any]] = []
    max_left = max(int(left_limit), round(line_h * 0.95))
    for x0, y0, x1, y1, _area in components:
        box_w = x1 - x0
        box_h = y1 - y0
        if box_w <= 0 or box_h <= 0 or x0 > max_left:
            continue
        aspect = box_w / max(1.0, float(box_h))
        if not 0.68 <= aspect <= 1.35:
            continue
        if box_w > line_h * 1.55 or box_h > line_h * 1.55:
            continue
        ...
        if (
            box_w >= line_h * 0.62
            and box_h >= line_h * 0.62
            and 0.23 <= density <= 0.40
            and central_ink <= 0.08
            and outer_fraction <= 0.065
        ):
            marker_type = "open_circle"
        elif (
            box_w >= line_h * 0.48
            and box_h >= line_h * 0.48
            and 0.62 <= density <= 0.93
            and central_ink >= 0.55
            and outer_fraction <= 0.08
        ):
...
def _match_visual_entry_markers_to_lines(
    ...
    """Pair visual ○/● markers with OCR rows by physical line geometry."""
    ...
    for marker_index, marker in enumerate(markers):
        ...
        for line_index, line in enumerate(lines):
            ...
            if vertical_overlap <= 0 and delta > line_h * 0.55:
                continue
...
def _parse_cjk_visual_marker_line(
    ...
    """Recover a marker-led CJK head when OCR omitted/misread the circle."""
    ...
    parsed = _parse_cjk_marker_pinyin_headword(
        synthetic, settings, profile,
    )
    if parsed is None:
        return None
