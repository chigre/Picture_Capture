...
第一次使用建议先完成【项目 Profile】和【检测版面参数】，然后只在一张典型页试画。不要一开始就修改高级阈值。

- `left_edge`：对应主界面的“运行普通画线”。适合词头靠近栏左缘、正文缩进稳定的版式；现在会根据自动/Otsu/自适应墨迹策略和当前栏实际墨迹密度判断，比旧版固定黑度阈值更能适应发黄、亮度变化和细字体扫描。
- `paddleocr`：对应“运行OCR画线”，适合词头需要通过文字、字号、粗体、词性或特殊符号来区分的版式。它会填入词头文字并生成坐标。主界面默认“使用有效缓存”，只有图像/模型发生变化或明确需要重跑时才选“重新OCR”。
...
- 主界面取消菜单栏/顶部工具栏；常用版面参数、OCR参数、辅助显示与主要操作集中在左栏。页面列表占用左栏剩余高度并可滚轮滚动；完整参数从【设置中心】打开，并按普通画线/OCR画线/显示校对/高级分区。
