...
版面行为中的【使用自动版面参数】默认关闭。开启后，每一页执行普通画线前都会先检测本页版面，并只用主界面勾选的字段（分栏数、正文起始Y、首栏X、单栏宽、栏间空、单行高、行间空）临时覆盖项目基准值；页面之间不会互相继承检测结果。【跟随词头列倾斜和局部变形】仍可独立按需开启。普通画线不再使用旧【手动分栏 / 手动Y】执行模式。
