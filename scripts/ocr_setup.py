...
# Backward-compatible numeric mapping used by existing tests/external callers.
PROFILES = {
    "1": CPU_PROFILE,
    "2": GPU_PROFILES[2],
    "3": GPU_PROFILES[1],
...
def platform_support(system: str | None = None, machine: str | None = None) -> dict[str, object]:
    system_name = (system or platform.system()).strip()
    machine_name = normalize_machine(machine or platform.machine())
    key = system_name.casefold()

    if key == "windows" and machine_name == "x86_64":
        return {
            "system": "Windows",
            "machine": machine_name,
            "paddle_cpu": True,
            "paddle_gpu": True,
            "note": "PaddlePaddle CPU and NVIDIA CUDA profiles are supported.",
        }
    if key == "linux" and machine_name == "x86_64":
        return {
            "system": "Linux",
            "machine": machine_name,
            "paddle_cpu": True,
    ...
    if key == "linux" and machine_name == "arm64":
        return {
            "system": "Linux",
            "machine": machine_name,
            "paddle_cpu": True,
            "paddle_gpu": False,
            "note": "PaddlePaddle CPU is supported; project GPU profiles target NVIDIA x86_64.",
        }
    if key == "darwin" and machine_name == "arm64":
        return {
            "system": "macOS",
            "machine": machine_name,
            "paddle_cpu": True,
            "paddle_gpu": False,
            "note": "PaddlePaddle supports Apple Silicon in CPU mode only.",
        }
    if key == "darwin":
        return {
            "system": "macOS",
            ...
            "paddle_cpu": False,
            "paddle_gpu": False,
            "note": "Current PaddlePaddle 3.3.x macOS wheels support arm64, not Intel x86_64.",
        }
    return {
        ...
        "paddle_cpu": False,
        "paddle_gpu": False,
        "note": "This platform is not in Picture Capture's tested PaddlePaddle support matrix.",
    }
...
def profile_supported(profile: dict[str, object], support: dict[str, object] | None = None) -> bool:
    current = support or platform_support()
    expect = profile.get("expect")
    if expect == "gpu":
        return bool(current.get("paddle_gpu"))
    if expect == "cpu":
        return bool(current.get("paddle_cpu"))
    return True
...
def recommended_profile(
    ...
    current = support or platform_support()
    if not current.get("paddle_cpu"):
        return CORE_PROFILE
    if not current.get("paddle_gpu"):
        return CPU_PROFILE
...
def show_hardware_recommendation(
    gpu_info: dict[str, object] | None,
    ...
    print(f"  OS: {current['system']} ({current['machine']})")
    print(f"  {current['note']}")

    if not current.get("paddle_gpu"):
        if current.get("paddle_cpu"):
            print("  Recommended: CPU OCR")
        else:
            print("  Recommended: Core only")
            print("  PaddleOCR/PaddlePaddle is not automatically installed on this platform.")
        return
    ...
    if profile.get("expect") == "gpu":
        ...
        print("  Installation will still be verified with a real Paddle GPU computation.")
    else:
        print("  Recommended: CPU OCR")
        gpu = primary_gpu(gpu_info)
...
def choose_profile() -> dict[str, object] | None:
    ...
    gpu_info = detect_nvidia_gpu() if support.get("paddle_gpu") else None
    recommendation = recommended_profile(gpu_info, support)
    show_hardware_recommendation(gpu_info, recommendation, support)
    ...
    if recommendation.get("expect") == "gpu":
    ...
    elif support.get("paddle_cpu"):
        print("  1. CPU OCR (Recommended)")
        print("  2. Google Lens only")
        print("  3. Core only")
        if support.get("paddle_gpu"):
            print("  4. Advanced: choose CUDA profile manually")
            prompt = "Selection [Enter=Recommended, 1-4]: "
        else:
            prompt = "Selection [Enter=Recommended, 1-3]: "
        ...
        if choice == "3":
            return CORE_PROFILE
        if choice == "4" and support.get("paddle_gpu"):
            return choose_advanced_gpu_profile()
    else:
        print("  1. Core only (Recommended)")
        print("  2. Google Lens only")
        choice = input("Selection [Enter=Recommended, 1-2]: ").strip()
        if choice in {"", "1"}:
            return CORE_PROFILE
        if choice == "2":
            return LENS_PROFILE
...
def install_profile(profile: dict[str, object]) -> int:
    support = platform_support()
    if not profile_supported(profile, support):
        print(f"ERROR: {profile['label']} is not supported on {support['system']} ({support['machine']}).")
        print(f"       {support['note']}")
        return 3
